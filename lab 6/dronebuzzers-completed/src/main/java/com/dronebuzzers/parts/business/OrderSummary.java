...
public class OrderSummary {
	...
	public OrderSummary(Order order, PartRepository partRepository) {
		...
		Iterator<OrderLine> orderLinesIterator = orderLines.iterator();
		while ( orderLinesIterator.hasNext()) {
	    	OrderLine orderLine = orderLinesIterator.next();
			if (orderLine.getCount() > 0) {
				Part part = partRepository.findByPartId(orderLine.getPartId());
				totalOrderAmount = totalOrderAmount
						+ part.getUnitPrice() * orderLine.getCount();
				orderSummaryLines
						.add(new OrderSummaryLine(part, orderLine.getCount()));
